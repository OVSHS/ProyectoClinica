...
bool validarIdentidad(const string& identidad) {
    // Validación básica - puede ajustarse según requisitos
...
public:
    ...
    Consulta(const string& fechaConsulta, const string& motivoConsulta, 
            ...
            const string& notasConsulta, int idMedicoConsulta) :
    ...
    // Crea una consulta desde una línea de texto
...
public:
    ...
    Medico(const string& nombre, const string& esp, const string& numColegiado) :
        idMedico(siguienteIdMedico++), nombreCompleto(nombre),
        especialidad(esp), numeroColegiacion(numColegiado), disponible(true) {}

    int obtenerId() const { return idMedico; }
    const string& obtenerNombre() const { return nombreCompleto; }
    const string& obtenerEspecialidad() const { return especialidad; }
...
/**
 * Estructura que representa una cola por especialidad
 */
...
public:
    ...
    void pacienteTomarTurno(int idPacienteSesion) {
        ...
        // Mostrar menú de médicos disponibles
        cout << "\nMedicos disponibles:\n";
    ...
    void guardarDatos() {
        ...
        cout << "Datos guardados automaticamente\n";
    }
    ...
    void cargarDatos() {
        ...
        while(getline(archivoPacientes, linea) && !linea.empty()) {
            pacientes.push_back(Paciente::desdeTexto(linea));
        }
        
        ifstream archivoMedicos("Medicos.txt");
        while(getline(archivoMedicos, linea) && !linea.empty()) {
            medicos.push_back(Medico::desdeTexto(linea));
        }
        
        ifstream archivoExpedientes("Expedientes.txt");
        while(getline(archivoExpedientes, linea)) {
            if(!linea.empty() && linea[0] == '#') {
                int idPaciente = stoi(linea.substr(1));
                Paciente* paciente = buscarPacientePorId(idPaciente);
                
                stringstream buffer;
                string temp;
                while(getline(archivoExpedientes, temp) && temp != "$") {
                    buffer << temp << "\n";
                }
                
                if(paciente) {
                    paciente->obtenerExpediente().cargarDesdeFlujo(buffer);
                }
            }
        }
...
void hiloReporteAutomatico(SistemaHospital* sistema) {
    while(programaActivo) {
        this_thread::sleep_for(chrono::minutes(7));
